...
@Component({
    ...
    templateUrl: './client-interviewer-dialog.component.html',
    styleUrls: ['../../client-detail/client-detail.component.css']
})
export class ClientInterviwerPannelDialog implements OnInit {

    constructor(
        public thisDialogRef: MatDialogRef<ClientInterviwerPannelDialog>, @Inject(MAT_DIALOG_DATA)
    ...
    public addInterviewerToPosition() {
        ...
        if (this.dataOption.modalType === 'add-client') {
            this.thisDialogRef.close(interviewerListArray);
        } else {
            //calling api to add
            this.clientServ.addSelectedInterviewerToClient(interviewerListArray, this.dataOption.clientId).pipe(first()).subscribe(response => {
                if (response.success === true) {
