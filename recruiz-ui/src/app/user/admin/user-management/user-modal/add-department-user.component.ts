...
@Component({
  ...
  templateUrl: './add-department-user.component.html',
  styleUrls: ['../user-management/user-management.component.css']
})
export class AddDepartmetHeadUserDialog implements OnInit {

  constructor(
    public thisDialogRef: MatDialogRef<AddDepartmetHeadUserDialog>, @Inject(MAT_DIALOG_DATA)
  ...
  public inviteDepartmentUser() {
    ...
    if (found) {
      if (emailUser === true) {
        this._user.inviteUser(this.data, this.dataOption.deptType).pipe(first()).subscribe((response: any) => {
          if (response.success) {
