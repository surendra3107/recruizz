...
public class AbstractRestClient {
	...
	@Value("${recruiz.connect.server.url}")
	protected String recruizConnectBaseUrl;
	...
	private final class LevelbarTokenRequestInterceptor implements ClientHttpRequestInterceptor {
		...
		public ClientHttpResponse intercept(final HttpRequest request, final byte[] body,
			...
			if (xAuthToken != null)
				request.getHeaders().add(ConnectURLConstants.CONNECT_AUTH_HEADER, xAuthToken);
	...
	protected <T> ResponseEntity<T> exchange(String relativeUrl, HttpMethod method, HttpEntity<?> requestEntity,
		...
		try {
			...
			return restTemplate.exchange(recruizConnectBaseUrl + relativeUrl, method, requestEntity, responseType);
		} catch (Throwable e) {
	...
	protected <T> T get(String reativeURL, MultiValueMap<String, String> paramMap, Class<T> type) throws PlutusClientException {
		try {
			UriComponents uriComponents = UriComponentsBuilder.fromUriString(recruizConnectBaseUrl + reativeURL)
