...
DROP TABLE IF EXISTS `team_user_target`;
...
CREATE TABLE IF NOT EXISTS `team_to_team_member` (
  `team_id` bigint(20) NOT NULL,
  `team_member_id` bigint(20) NOT NULL,
  PRIMARY KEY (`team_id`,`team_member_id`),
  KEY `FKmib1qqq7hkvk9inlnkhuolps2` (`team_member_id`),
  CONSTRAINT `FKikdavn40tj2ywof6a523igce7` FOREIGN KEY (`team_id`) REFERENCES `team` (`id`),
  CONSTRAINT `FKmib1qqq7hkvk9inlnkhuolps2` FOREIGN KEY (`team_member_id`) REFERENCES `team_member` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
